...
ARG IPFS_GATEWAY
RUN echo ${IPFS_GATEWAY}

ARG PINATA_API_KEY
RUN echo ${PINATA_API_KEY}

ARG PINATA_SECRET_API_KEY
RUN echo ${PINATA_SECRET_API_KEY}
...
# Install all dependencies for the workspace, including common and algo-backend
...
# Build the algo-backend package
# RUN pnpm --filter algo-backend build:all_repo
# RUN pnpm --filter algo-backend build:all 
# RUN pnpm --filter algo-backend build:prisma
# RUN pnpm --filter algo-backend build
...
COPY --from=base /app/packages/indexer-v2-db  ./node_modules/indexer-v2-db
COPY --from=base /app/apps/algo-backend/node_modules ./apps/algo-backend/node_modules
COPY --from=base /app/apps/algo-backend/dist ./apps/algo-backend/dist

# Copy only necessary files for the application to run
COPY apps/algo-backend/package.json ./
...
CMD ["node", "apps/algo-backend/dist/index.js"]
