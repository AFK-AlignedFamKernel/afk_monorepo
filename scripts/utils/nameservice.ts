...
dotenv.config();
const PATH_NAMESERVICE = path.resolve(
  __dirname,
  "../../onchain/cairo/target/dev/afk_Nameservice.contract_class.json"
);
const PATH_NAMESERVICE_COMPILED = path.resolve(
  __dirname,
  "../../onchain/cairo/target/dev/afk_Nameservice.compiled_contract_class.json"
);
...
export const createNameservice = async (owner:string, admin:string,
  ...
  try {
    ...
    const compiledSierraNamespace = json.parse(
      fs.readFileSync(PATH_NAMESERVICE).toString("ascii")
    );
    // console.log("compiledSierraNamespace",compiledSierraNamespace)

    const compiledNamespaceCasm = json.parse(
      fs.readFileSync(PATH_NAMESERVICE_COMPILED).toString("ascii")
    );
