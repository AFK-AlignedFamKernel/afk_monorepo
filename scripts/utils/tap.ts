...
const PATH_TAP_QUEST = path.resolve(
  __dirname,
  "../../onchain/cairo/target/dev/afk_TapQuests.contract_class.json"
);
const PATH_TAP_QUEST_COMPILED = path.resolve(
  __dirname,
  "../../onchain/cairo/target/dev/afk_TapQuests.compiled_contract_class.json"
);
...
export const createTapQuests = async () => {
  try {
    ...
    let TapClassHash = process.env.TAP_CLASS_HASH as string;

    const compiledSierraAAaccount = json.parse(
      fs.readFileSync(PATH_TAP_QUEST).toString("ascii")
    );
    const compiledAACasm = json.parse(
      fs.readFileSync(PATH_TAP_QUEST_COMPILED).toString("ascii")
    );
    ...
    if (process.env.REDECLARE_CONTRACT == "true") {
      ...
      TapClassHash = contractClassHash;
    ...
    console.log("Error createTapQuests= ", error);
  }
};
