...
export const deployToken = async () => {
    ...
    let token_address: string | undefined = ESCROW_ADDRESS[constants.StarknetChainId.SN_SEPOLIA] as any // change default address
    const privateKey0 = process.env.DEV_PK as string;
    const accountAddress0 = process.env.DEV_PUBLIC_KEY as string;
    const account = new Account(provider, accountAddress0, privateKey0, "1");
    const total_supply=100_000_000;
    if (process.env.IS_DEPLOY_CONTRACT == "true") {
        let launchpadContract = await createToken(
            total_supply,
            "UHOH",
            "UHOHLFG"
        );
        ...
        if (launchpadContract?.contract_address) {
            ...
            token = await prepareAndConnectContract(token_address ?? launchpadContract?.contract_address, account)
        }
