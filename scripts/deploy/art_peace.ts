...
export const deployArtPeace = async () => {
    let art_peace_address: string | undefined = ART_PEACE_ADDRESS[constants.StarknetChainId.SN_SEPOLIA] as any // change default address
    ...
    if (process.env.IS_DEPLOY_CONTRACT == "true") {
        ...
        if (artPeaceContract?.contract_address) {
            ...
            art_peace = await prepareAndConnectContract(art_peace_address ?? artPeaceContract?.contract_address, account)
        }
