...
export class WalletManager {
  ...
  private static async createAndStoreKeyPairWithCredential(
    credential?: Credential | null,
    propsPasskey?: GeneratePasskeyValues,
  ): Promise<{
    secretKey: string;
    publicKey: string;
    strkPrivateKey?: string;
    ...
    //
    await this.storeSecretKeyWithCredential(
      secretKey,
      publicKey,
      strkPrivateKey,
      starkKeyPub,
  ...
  private static async storeSecretKey(
    ...
    try {
      ...
      if (credential && credential.type === 'public-key') {
        ...
        localStorage.setItem(`${WalletManager.SALT_KEY_PREFIX}`, JSON.stringify(Array.from(salt)));
        ...
        localStorage.setItem(`${WalletManager.IS_WALLET_SETUP}`, 'true');
      } else {
