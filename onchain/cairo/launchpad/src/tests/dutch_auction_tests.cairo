...
mod tests {
    ...
    fn test_constructor_valid_deployment() {
        ...
        assert(total_supply == u256 { low: 1_000_000, high: 0 } * TEN_POW_18, "Invalid total supply");
    ...
    fn test_claim_unsold_happy_path() {
        ...
        let contract_sale_balance_after = setup.token_sale.balance_of(setup.auction_addr);
        let (_, _, _, _, _, _, _, final_total_supply, final_sold) = setup.auction.get_auction_details();

        assert(owner_sale_balance_after == owner_sale_balance_before + expected_unsold, "Owner bal wrong");
        assert(contract_sale_balance_after == contract_sale_balance_before - expected_unsold, "Contract bal wrong");
