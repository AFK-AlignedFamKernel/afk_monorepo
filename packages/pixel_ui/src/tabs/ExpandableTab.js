...
const ExpandableTab = (props) => {
  ...
  return (
    <div
      className={'ExpandableTab' + (props.expanded ? ' ExpandableTab--expanded' : '')}
      onMouseDown={handleMouseDown}
    >
      <h1 className="Text__xlarge Heading__main ExpandableTab__title">{props.title}</h1>
      <div className="ExpandableTab__content">
        {(!props.expanded || (props.expanded && !props.isMobile)) && <MainSection {...rest} />}
        {props.expanded &&
          !props.isMobile &&
      ...
      {(props.canExpand === undefined || props.canExpand) && (
        ...
        >
          <img src={HamburgerIcon} alt="Expand" className="ExpandableTab__hamburger" />
        </div>
      )}
      <p className="Button__close ExpandedTab__close" onClick={() => props.setActiveTab('Canvas')}>
        X
      </p>
    </div>
  );
