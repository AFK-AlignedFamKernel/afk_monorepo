...
async function getFunkitStripeCheckoutQuote(fastify: FastifyInstance, _options: RouteOptions) {
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const { getCheckoutQuote, getStripeBuyQuote } = await import('@funkit/api-base');
        ...
        const stripeQuote = stripeFullQuote?.destination_network_quotes?.[sourceAsset.network]?.[0];
        if (!stripeQuote) {
        ...
        const finalQuote = {
          ...
          networkFees: (
            Number(stripeQuote.fees.network_fee_monetary) + Number(baseQuote.estFeesUsd)
          ).toFixed(2),
