...
async function getFunkitStripeCheckoutStatus(fastify: FastifyInstance, _options: RouteOptions) {
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        const { getCheckoutByDepositAddress } = await import('@funkit/api-base');
