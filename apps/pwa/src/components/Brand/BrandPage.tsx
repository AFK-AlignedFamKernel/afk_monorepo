...
export default function BrandPage({ slug_name }: { slug_name: string }) {
    ...
    return (
        <div className="w-full flex justify-center items-start px-0 sm:px-2 py-4">
            <div className="w-full sm:max-w-2xl bg-white/90 dark:bg-black/80 rounded-xl shadow-lg p-2 sm:p-6 mx-0 sm:mx-auto flex flex-col gap-4">
                ...
                {brand && (
                    <div className="flex flex-col items-center gap-4 break-words whitespace-normal w-full">
                        <div className="flex flex-row items-center gap-2 items-baseline w-full justify-center">
                            <h2 className="text-2xl font-bold text-center break-words w-full text-black dark:text-white">{brand?.name} </h2>
                        ...
                        <div className="w-full flex justify-center">
                            <div className="flex flex-row gap-2">
                                <Link href={`https://x.com/${brand.twitter_handle}`} target="_blank">
                                    <button className="flex flex-row items-center gap-2 px-3 py-1 rounded hover:bg-blue-200 transition bg-white/70 dark:bg-black/40 border border-gray-200 dark:border-gray-700">
                ...
                <div className="flex flex-row gap-2 sm:gap-4 w-full justify-center my-4">
                    <button className={`px-4 py-2 rounded-md w-1/2 sm:w-auto ${activeTab === "leaderboard" ? "bg-blue-700 text-white" : "border border-gray-300 bg-white/70 dark:bg-black/40 text-gray-800 dark:text-gray-200"}`} onClick={() => setActiveTab("leaderboard")}>Leaderboard</button>
                    <button className={`px-4 py-2 rounded-md w-1/2 sm:w-auto ${activeTab === "feed" ? "bg-blue-700 text-white" : "border border-gray-300 bg-white/70 dark:bg-black/40 text-gray-800 dark:text-gray-200"}`} onClick={() => setActiveTab("feed")}>Feeds</button>
                </div>
                            ...
                            <div className="flex flex-row gap-2 sm:gap-4 overflow-x-auto shadow-md rounded-md p-2 mb-4 w-full scrollbar-hide">
                                {leaderboards?.map((leaderboard: any) => (
                                    <div key={leaderboard.id}
                                        className={`border border-gray-300 rounded-md p-2 min-w-[100px] flex flex-row items-center gap-2 cursor-pointer transition ${activePlatformLeaderboard === leaderboard.platform ? "bg-blue-700 text-white" : "bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700"}`}
                            ...
                            {leaderboard && (
                                <div className="overflow-x-auto scrollbar-hide w-full">
                                    ...
                                    <div className="flex flex-wrap gap-2 sm:gap-4 mb-2">
                                        <p className="rounded px-2 py-1 text-sm bg-white/70 dark:bg-black/40">Total Score: <span className="font-bold">{leaderboard.total_score}</span></p>
                                        <p className="rounded px-2 py-1 text-sm bg-white/70 dark:bg-black/40">Total Users: <span className="font-bold">{leaderboard.total_users}</span></p>
                                        <p className="rounded px-2 py-1 text-sm bg-white/70 dark:bg-black/40">Rank Position: <span className="font-bold">{leaderboard.rank_position}</span></p>
                                    </div>
                                    <div className="overflow-x-auto rounded w-full">
                                        <table className="min-w-full w-full text-sm bg-white/80 dark:bg-black/60 rounded-lg">
                                            <thead className="sticky top-0 z-10 border-b border-gray-300 dark:border-gray-700 bg-white/90 dark:bg-black/80">
                                                <tr>
