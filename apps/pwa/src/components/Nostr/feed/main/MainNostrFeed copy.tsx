...
const MainNostrFeed: React.FC<MainNostrFeedProps> = ({
  ...
  // Intersection observer for infinite scroll - attached to last item
  ...
  // Intersection observer for infinite scroll - similar to NostrFeed
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      if (loaderRef.current) {
        console.log('Observing loaderRef element');
        observer.observe(loaderRef.current);
      } else {
        console.log('loaderRef.current is null - no element to observe');
      }
    }, 100);
    ...
    return () => {
      ...
      if (loaderRef.current) {
        console.log('Unobserving loaderRef element');
  ...
  const renderNotes = (notes: TrendingNote[] | ViralNote[] | ScrapedNote[] | TopAuthor[] | TrendingTopAuthor[]) => {
    ...
    return filteredNotes.map((note, index) => {
      ...
      if (isLastItem) {
        console.log('Attaching loaderRef to last item:', {
          index,
