  ...
  const voteTipMutation = useMutation({
    mutationFn: async ({ voteParams, contractAddress }: { 
      ...
      try {
        ...
        // const tx = await account.execute([approveCallData, vote]);
        ...
        console.log('Vote transaction hash:', mockTx.transaction_hash);
        return mockTx;
  ...
  const voteStarknetOnlyMutation = useMutation({
    mutationFn: async ({ voteParams, contractAddress }: { 
      ...
      try {
        ...
        console.log('StarkNet only vote transaction hash:', mockTx.transaction_hash);
        return mockTx;
