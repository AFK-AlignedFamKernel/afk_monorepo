...
export class PasskeyManager {
  ...
  public static async generatePasskeyAndSave(
    props: GeneratePasskeyValues,
  ...
  public static async storeSecretKey(credentialProps: Credential): Promise<void> {
    try {
      const encoder = new TextEncoder();
      ...
      if (credential && credential.type === 'public-key') {
        ...
        localStorage.setItem(`${PasskeyManager.IS_WALLET_SETUP}`, 'true');

        localStorage.setItem(`${PasskeyManager.SALT_KEY_PREFIX}`, JSON.stringify(Array.from(salt)));

        localStorage.setItem(`${PasskeyManager.SALT_KEY_PREFIX}`, JSON.stringify({rawId}));
      } else {
