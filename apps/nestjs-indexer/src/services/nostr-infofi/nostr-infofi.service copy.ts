...
export class NostrInfofiService {
  ...
  async createTipUserWithVote(data: NostrInfofiInterface) {
    try {
      ...
      // First, ensure the overall_data exists
      ...
      // Then create/update the epoch_data
      ...
      let profileData = await this.prismaService.profile_data.upsert({
        where: {
          nostr_id: nostrAddressString,
        },
        update: {
  ...
  async handleNewEpochEvent(data: NewEpochEventInterface) {
    try {
      ...
      await this.prismaService.overall_data.upsert({
        where: {
          contract_address: data.contract_address,
        },
        update: {
